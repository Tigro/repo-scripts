...
KPATH="/mirror/fedora/linux/updates/12/i386/"
...
ARCH="i386 x86_64"
BUILDPATTH="/mirror/fedora/russianfedora/build/12"
...
for repo in $REPOS; do
	...
	if [ "$repo" == "rpmfusion-nonfree-updates" -o "$repo" == "rpmfusion-free-updates" -o "$repo" == "rpmfusion-nonfree" -o "$repo" == "rpmfusion-free" ]; then
	...
	if [ "$repo" == "russianfedora" -o "$repo" == "russianfedora-updates" -o "$repo" == "russianfedora-updates-testing" ]; then
        ...
        /usr/sbin/chroot /root/fedora-chroot/ /usr/bin/createrepo  $PARAM $EXCLUDERPMS -x *debuginfo* -d --update -g $BUILDPATTH/$repo/$arch/$repo.xml \
