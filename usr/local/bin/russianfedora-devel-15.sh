...
KPATH="/mirror/fedora/linux/development/15/i386/os/Packages/"
...
case "$1" in
	...
	;;
    -a)
	REPOS="rpmfusion-free rpmfusion-nonfree russianfedora russianfedora-updates russianfedora-updates-testing"
	;;
    -u)
...
ARCH="i386 x86_64"
BUILDPATTH="/mirror/fedora/russianfedora/build/development/15/"
    ...
    for arch in $ARCH; do
		...
		rpmfusion-nonfree)
	            DPATH="/mirror/fedora/rpmfusion/nonfree/fedora/development/$arch/os/"
                    ;;
	    	russianfedora)
 		    DPATH="/mirror/fedora/linux/development/15/$arch/os/Packages/"
		    ;;
		russianfedora-updates)
	            DPATH="/mirror/fedora/linux/updates/15/$arch"
		    ;;
		russianfedora-updates-testing)
		    DPATH="/mirror/fedora/linux/updates/testing/15/$arch"
		    ;;
	    	*)
		    exit 0
		    ;;
		...
		/usr/sbin/chroot /root/fedora-chroot/ /usr/bin/createrepo  $PARAM $EXCLUDERPMS -x *debuginfo* -d --update -g /root/comps/15/$repo.xml -o $BUILDPATTH/$repo/$arch/ \
                    -u $URLPATH $DPATH
    done
done
cd -
